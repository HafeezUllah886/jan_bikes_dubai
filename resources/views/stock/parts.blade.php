...
@section('content')
    <div class="row">
        <div class="col-12">
            ...
            {{-- Filter Bar --}}
            <form method="GET" action="{{ route('stock.parts') }}" class="mb-3">
                <div class="row g-2 align-items-center">
                    ...
                    @foreach(['Available' => 'Available', 'Sold' => 'Sold', 'all' => 'All'] as $val => $label)
                        <div class="col-auto">
                            <a href="{{ route('stock.parts', ['status' => $val]) }}"
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">
                        Parts Stock
                        <span class="badge {{ $status === 'Available' ? 'bg-success' : ($status === 'Sold' ? 'bg-secondary' : 'bg-primary') }} ms-1">
                        ...
                        <tbody>
                            @foreach ($items as $key => $item)
                                <tr>
                                    ...
                                    <td>{{ $item->qty }}</td>
                                    <td class="fw-semibold">{{ number_format($item->net_cost, 2) }}</td>
                                    <td class="text-info fw-semibold">{{ number_format($item->sale_price, 2) }}</td>
                                    <td>
                                        @if ($item->status === 'Available')
                                            <span class="text-muted">—</span>
                                        @elseif ($item->profit_loss >= 0)
                                            <span class="text-success fw-bold">+{{ number_format($item->profit_loss, 2) }}</span>
                                        @else
                                            <span class="text-danger fw-bold">{{ number_format($item->profit_loss, 2) }}</span>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="{{ route('part_purchase.show', $item->id) }}"
